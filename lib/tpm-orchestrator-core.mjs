...
export function runOrchestratorCycle(){
  ...
  const scenarioCoverage = pct(scenarioSignals.filter(Boolean).length, scenarioSignals.length);
  ...
  const overallProgress = Math.round((orchestrator + planner + scenarioCoverage + knowledge + continuity) / 5);
  ...
  const result = {
    ...
    nextWave: [
      ...
      { slug:"scenario-switching", title:"scenario switching", progress: scenarioCoverage, status:"active" },
