...
import {
  CONNECTOR_IDS,
  buildConnectorRuntime,
  stampConnectorValidation,
  syncConnectorRegistry
} from "./connector-readiness.js";
...
function buildPersistenceFingerprint(state = {}) {
  return stableJoin([
    ...
    [
      state.connectors?.lastValidationSweepAt || "",
      state.connectors?.lastRecheckAt || "",
      state.connectors?.lastAction || ""
    ],
    Object.entries(state.connectors?.records || {})
      .sort(([left], [right]) => left.localeCompare(right))
      .map(([key, value]) =>
        stableJoin([
          key,
...
function buildControlCenter(state, provider, recovery, connectorRuntime) {
  const refreshPolicy = getRefreshPolicy(state.controls.refreshPolicyKey);
  const connectorSummaryCards = [
    connectorRuntime.summary.paperReady,
    connectorRuntime.summary.liveDataReady,
    connectorRuntime.summary.brokerReady,
    connectorRuntime.summary.alertReady
  ];
  ...
  return {
    ...
    connectorReadiness: {
      overall: connectorRuntime.summary.overall,
      summaryCards: connectorSummaryCards,
      counts: connectorRuntime.summary.counts,
      lastValidationSweepAt: connectorRuntime.summary.lastValidationSweepAt,
      lastRecheckAt: connectorRuntime.summary.lastRecheckAt,
      connectors: connectorRuntime.connectors.map((connector) => ({
        id: connector.id,
        name: connector.name,
        category: connector.category,
        readinessLabel: connector.readinessLabel,
        readinessTone: connector.readinessTone,
        operatorRecommendation: connector.operatorRecommendation,
        lastValidationLabel: connector.lastValidationLabel
      })),
      operatorRecommendations: connectorRuntime.operatorRecommendations
    },
    actions: [
      {
        ...
        danger: "positive",
        detail: "Runs local-safe connector validation across market-data, broker, notifications, and Telegram placeholders."
      },
      {
        id: "recheck-connectors",
        label: "Recheck connectors",
        danger: "neutral",
        detail: "Re-evaluates the current connector registry and refreshes operator readiness timestamps."
      },
      {
        id: "safe-reset",
        label: "Safe reset",
...
function buildDiagnostics(state, provider, chart, decisionSnapshot, assistant, recovery, connectorRuntime) {
  ...
  if (state.providerKey === "local-bridge" && connectorRuntime.summary.liveDataReady.code !== "ready") {
    issues.push({
      label: "Live-data placeholder not ready",
      severity: connectorRuntime.summary.liveDataReady.code === "error" ? "warning" : "info",
      detail: connectorRuntime.summary.liveDataReady.detail,
      action: "Return to the demo feed or repair the bridge payload before relying on the live-data placeholder."
    });
  }

  if (connectorRuntime.summary.brokerReady.code !== "ready") {
    issues.push({
      label: "Broker readiness remains blocked",
      severity: "info",
      detail: connectorRuntime.summary.brokerReady.detail,
      action: "Keep the terminal in paper mode. Broker activation is intentionally disabled."
    });
  }
  ...
  const integrationReadiness = {
    paperReady: connectorRuntime.summary.paperReady,
    liveDataReady: connectorRuntime.summary.liveDataReady,
    brokerReady: connectorRuntime.summary.brokerReady,
    alertReady: connectorRuntime.summary.alertReady,
    overall: connectorRuntime.summary.overall,
    signature: connectorRuntime.summary.signature
  };
  ...
  return {
    ...
    signature: stableJoin([
      ...
      integrationReadiness.signature,
...
async function buildDeskSnapshot({ selectedSymbol, selectedTimeframe, persistMode = "changed" } = {}) {
  ...
  const connectorRuntime = await buildConnectorRuntime({
    state,
    provider: quoteCollection.provider,
    providerCatalog: getMarketProviderCatalog()
  });
  state.connectors = syncConnectorRegistry(state.connectors, connectorRuntime.connectors);
  const controlCenter = buildControlCenter(state, quoteCollection.provider, recovery, connectorRuntime);
  ...
  const overview = {
    ...
    connectorReadiness: connectorRuntime.connectors,
    controlCenter,
  ...
  overview.overviewSignature = stableJoin([
    ...
    connectorRuntime.connectors.map((item) =>
      stableJoin([item.id, item.readinessStatus, item.configured ? 1 : 0, item.lastValidationTime || "", item.blockedReason || "", item.unsupportedReason || ""])
    ),
    connectorRuntime.summary.signature,
    stableJoin([
      selectedMarket.symbol,
      selectedMarket.bias,
...
export async function runTradingTerminalControlAction(payload = {}) {
  ...
  if (action === "validate-readiness") {
    state.connectors = stampConnectorValidation(state.connectors, CONNECTOR_IDS, now, "validate");
    state.controls.lastAction = "Validate connector readiness";
    appendActivity(
      state,
      "Connector readiness validated",
      "Operator Control Center",
      "Validated",
      "Local-safe connector validation ran across market-data, broker, notifications, and Telegram placeholders."
    );
    appendAudit(state, "INFO", "Operator validated connector readiness in local-safe mode.");
    writeDeskState(state);
    return {
      ok: true,
      notice: "Connector readiness validation completed in local-safe mode.",
      overview: await buildDeskSnapshot({ persistMode: "changed" })
    };
  }

  if (action === "recheck-connectors") {
    state.connectors = stampConnectorValidation(state.connectors, CONNECTOR_IDS, now, "recheck");
    state.controls.lastAction = "Recheck connectors";
    appendActivity(
      state,
      "Connector readiness rechecked",
      "Operator Control Center",
      "Rechecked",
      "Connector registry and local-safe readiness checks were re-evaluated."
    );
    appendAudit(state, "INFO", "Operator rechecked connector readiness.");
    writeDeskState(state);
    return {
      ok: true,
      notice: "Connector readiness was rechecked against the current local configuration.",
      overview: await buildDeskSnapshot({ persistMode: "changed" })
    };
  }
