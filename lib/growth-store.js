...
New-Item -ItemType Directory -Force `
  ...
  ".\app\growth" `
  ".\app\client-portal" `
  ".\app\api\growth\status" `
  ".\app\api\growth\plans" `
  ".\app\api\growth\customers" `
  ".\app\api\growth\subscriptions" `
  ".\app\api\growth\onboarding" `
  ".\app\api\growth\portal" | Out-Null
...
const DEFAULT_GROWTH = {
  plans: [
    ...
    },
    {
      ...
      tier: "pro",
      seats: 3,
      features: ["workspace", "execution", "portfolio lab", "analytics", "risk"]
    },
    {
      id: "PLAN-INSTITUTION",
      name: "Institution",
      ...
      tier: "institution",
      seats: 10,
      features: ["broker hub", "ops", "cloud", "priority rollout", "backbone"]
    }
  ],
  customers: [],
  subscriptions: [],
...
function normalize(x) {
  return {
    plans: Array.isArray(x?.plans) ? x.plans : DEFAULT_GROWTH.plans,
...
export function createCustomer(payload = {}) {
  ...
  return mutateGrowth((draft) => {
    ...
    let flow = draft.onboarding.find((x) => x.customerId === customer.id);
    if (!flow) {
