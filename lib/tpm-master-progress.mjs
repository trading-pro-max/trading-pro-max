...
function getTradingChecks() {
  return [
    ...
    { key: "paper_live", label: "Paper and live modes", weight: 1, pass: exists("trading-core/paper-live/modes.ts") },
    ...
    { key: "trading_workflow", label: "Trading workflow scaffold", weight: 1, pass: exists("ops/workflows/trading-core.yml") }
  ];
}

export function getMasterProgressStatus() {
  const config = readJson(CONFIG_FILE, {
    baseGlobalProgress: 58,
    productionJumpMax: 14,
    tradingJumpMax: 12,
    targetGlobalProgress: 84
  });

  const productionChecks = getProductionChecks();
  const tradingChecks = getTradingChecks();

  const productionEarned = productionChecks.filter(x => x.pass).reduce((a, b) => a + b.weight, 0);
  const productionMax = productionChecks.reduce((a, b) => a + b.weight, 0);
  const tradingEarned = tradingChecks.filter(x => x.pass).reduce((a, b) => a + b.weight, 0);
  const tradingMax = tradingChecks.reduce((a, b) => a + b.weight, 0);

  const globalProgress = Math.min(
    config.targetGlobalProgress,
    config.baseGlobalProgress + Math.min(productionEarned, config.productionJumpMax) + Math.min(tradingEarned, config.tradingJumpMax)
  );
  ...
  const result = {
    ...
    targetGlobalProgress: config.targetGlobalProgress,
    production: {
      earned: productionEarned,
      max: productionMax,
      closed: productionEarned === productionMax,
      checks: productionChecks
    },
    trading: {
      earned: tradingEarned,
      max: tradingMax,
      closed: tradingEarned === tradingMax,
      checks: tradingChecks
    },
...
export function syncMasterReadme() {
  const status = getMasterProgressStatus();
  ...
  const section = [
    ...
    `- Production Core jump: ${status.production.earned}/${status.production.max}`,
    `- Trading Core jump: ${status.trading.earned}/${status.trading.max}`,
...
export function getTradingCoreStatus() {
  ...
  return {
    ...
    localCertified: status.localCertified,
    modulesClosed: status.modulesClosed,
    modulesTotal: status.modulesTotal,
    languages: status.languages,
    updatedAt: status.updatedAt
  };
}

export function runTradingCoreCycle() {
  syncMasterReadme();
  return getTradingCoreStatus();
}
