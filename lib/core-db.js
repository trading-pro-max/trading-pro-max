...
function normalizeBroker(broker) {
  return {
    id: broker?.id || makeId("BRK"),
    key: broker?.key || "CUSTOM",
    name: broker?.name || "Custom Broker",
    category: broker?.category || "custom",
    status: broker?.status || "READY",
    connected: Boolean(broker?.connected),
    mode: broker?.mode || "SANDBOX",
    coverage: Array.isArray(broker?.coverage) ? broker.coverage : [],
    lastSyncAt: broker?.lastSyncAt || null
  };
}
...
function normalizeDb(db) {
  return {
    ...
    brokers: Array.isArray(db?.brokers) ? db.brokers.map(normalizeBroker) : DEFAULT_DB.brokers.map(normalizeBroker)
  };
}
