...
export default function Page(){
  const data=readJson(path.join(process.cwd(),"data","live","ops.json"),{streams:[],watches:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Live Ops</h1>
        </div>
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Watches</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12}}>
            {Object.entries(data.watches || {}).map(([k,v])=>(
              <div key={k} style={box}>
