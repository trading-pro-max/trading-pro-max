...
export default function Page(){
  const data=readJson(path.join(process.cwd(),"data","infinity","sessions.json"),{loops:[],commands:[],totals:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Session Lab</h1>
        </div>

        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Tracked Loops</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:12}}>
            {(data.loops || []).map((x)=>(
              <div key={x.file} style={box}>
                <div style={{fontWeight:900}}>{x.slug}</div>
                <div style={{marginTop:8,color:"#22c55e",fontWeight:900,fontSize:20}}>{x.pid || "N/A"}</div>
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Totals</div>
