...
export function ProductWorkspaceChrome({ children, title, actions = [] }) {
  ...
  const pinnedContinuityReports = useMemo(
    () =>
      pinnedContinuityInsightReportIds
        .map((id) => continuityInsightReports.find((report) => report.id === id))
        .filter(Boolean),
    [continuityInsightReports, pinnedContinuityInsightReportIds]
  );
  const pinnedContinuityCollections = useMemo(
    () =>
      pinnedContinuityInsightCollectionIds
        .map((id) => continuityInsightCollections.find((collection) => collection.id === id))
        .filter(Boolean),
    [continuityInsightCollections, pinnedContinuityInsightCollectionIds]
  );
  const pinnedContinuityPairs = useMemo(
    () =>
      pinnedContinuityInsightPairs
        .map((pair) => ({
          ...pair,
          primaryReport: continuityInsightReports.find((report) => report.id === pair.primaryReportId) || null,
          comparisonReport: continuityInsightReports.find((report) => report.id === pair.comparisonReportId) || null
        }))
        .filter((pair) => pair.primaryReport),
    [continuityInsightReports, pinnedContinuityInsightPairs]
  );
  const promotedContinuityCheckpoints = useMemo(
    () =>
      promotedContinuityInsightCheckpoints
        .map((promotion) => ({
          ...promotion,
          collection: continuityInsightCollections.find((collection) => collection.id === promotion.collectionId) || null,
          report: continuityInsightReports.find((report) => report.id === promotion.reportId) || null
        }))
        .filter((promotion) => promotion.collection && promotion.report),
    [continuityInsightCollections, continuityInsightReports, promotedContinuityInsightCheckpoints]
  );
  ...
  const rankedHomeBaseContexts = homeBaseSummary.topContexts || [];
  const rankedQuickActions = homeBaseSummary.quickActions || [];
  const rankedResumeOptions = homeBaseSummary.resumeOptions || [];
  const rankedModuleSwitches = homeBaseSummary.moduleSwitches || coreModules;
  ...
  function handleOpenPinnedContinuityCollection(collection) {
    if (!collection) return;
    loadContinuityInsightCollection(collection.id);
    openJournalWorkspace();
  }

  function handleOpenPinnedContinuityPair(pair) {
    if (!pair) return;
    openPinnedContinuityInsightPair(pair.id);
    openJournalWorkspace();
  }

  function handleOpenPromotedContinuityCheckpoint(promotion) {
    if (!promotion) return;
    openPromotedContinuityInsightCheckpoint(promotion.id);
    openJournalWorkspace();
  }

  function handleHomeBaseCommand(action) {
    const commandKey = String(action?.commandKey || "").trim();
    const source = String(action?.source || "").trim();

    if (commandKey === "resume-best-context") {
      handleHomeBaseCommand(homeBaseSummary.resumeTarget);
      return;
    }
    ...
    if (commandKey === "open-continuity-posture") {
      if (latestHandoffNote) {
        applyHandoffContext(latestHandoffNote);
        return;
      }
      if (latestContinuityInsightCollection) {
        handleOpenPinnedContinuityCollection(latestContinuityInsightCollection);
        return;
      }
      if (activeContinuityInsightReport) {
        handleOpenContinuityInsightReport(activeContinuityInsightReport);
        return;
      }
      openJournalWorkspace();
      return;
    }
    if (commandKey === "resume-handoff" && latestHandoffNote) {
      applyHandoffContext(latestHandoffNote);
      return;
    }
    if (commandKey === "resume-continuity-collection" && latestContinuityInsightCollection) {
      handleOpenPinnedContinuityCollection(latestContinuityInsightCollection);
      return;
    }
    if (commandKey === "resume-replay-queue" && activeReplayQueue) {
      setActiveReplayQueue(activeReplayQueue.id);
      openJournalWorkspace();
      return;
    }
    if (commandKey === "resume-review-snapshot" && activeReviewSnapshot) {
      loadReviewSnapshot(activeReviewSnapshot.id);
      openJournalWorkspace();
      return;
    }
    if (commandKey === "resume-continuity-report" && activeContinuityInsightReport) {
      handleOpenContinuityInsightReport(activeContinuityInsightReport);
      return;
    }
    if (commandKey === "open-market-intelligence" || commandKey === "open-watchlists" || source === "watchlist") {
      openPath("/market-intelligence");
      return;
    }
    if (commandKey === "open-execution-center") {
      openPath("/execution-center");
      return;
    }
    if (commandKey === "open-risk-control") {
      openPath("/risk-control");
      return;
    }
    if (commandKey === "open-ai-copilot") {
      openPath("/ai-copilot");
      return;
    }
    if (commandKey === "switch-module") {
      openPath(action?.path);
      return;
    }
    if (source === "handoff" && latestHandoffNote) {
      applyHandoffContext(latestHandoffNote);
      return;
    }
    if (source === "continuity-collection" && latestContinuityInsightCollection) {
      handleOpenPinnedContinuityCollection(latestContinuityInsightCollection);
      return;
    }
    ...
    if (source === "continuity-report" && activeContinuityInsightReport) {
      handleOpenContinuityInsightReport(activeContinuityInsightReport);
      return;
    }
      ...
      <section style={{ ...createSurface({ level: "hero", accent: "info", padding: 22, radius: "xxl" }) }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 18, alignItems: "start" }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {rankedModuleSwitches.map((module) => (
                <Link key={module.href} href={module.href} style={{ ...createButton({ compact: true, tone: module.tone }), minWidth: 158, justifyContent: "center" }}>
                  {module.rank ? `${module.rank}. ` : ""}{module.label}
                </Link>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <div style={{ ...createSurface({ level: "elevated", accent: "info", padding: 16, radius: "xl" }) }}>
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: homeBaseSummary.overallTone || "info", padding: 12, radius: "lg" }), marginTop: 14, display: "grid", gap: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <div>
                    <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.6, fontWeight: 800 }}>Home Base Summary</div>
                    <div style={{ fontSize: 18, fontWeight: 900, marginTop: 6 }}>
                      {homeBaseSummary.overallState} | {homeBaseSummary.overallScore}%
                    </div>
                  </div>
                  <span style={createPillStyle(homeBaseSummary.resumeTarget?.tone || homeBaseSummary.overallTone || "info")}>
                    {homeBaseSummary.resumeTarget?.actionLabel || "Desk Ready"}
                  </span>
                </div>
                <div style={{ color: deskTheme.colors.soft, lineHeight: 1.6 }}>{homeBaseSummary.summary}</div>
                <div style={{ color: "#f8fafc", fontWeight: 800 }}>
                  Resume target: {homeBaseSummary.resumeTarget?.focusLabel || homeBaseSummary.resumeTarget?.label || "Execution Center"}
                </div>
                <div style={{ display: "grid", gap: 10 }}>
                  <div>
                    <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Ranked Quick Actions</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      {rankedQuickActions.slice(0, 4).map((action) => (
                        <button
                          key={action.key}
                          type="button"
                          onClick={() => handleHomeBaseCommand(action)}
                  ...
                  <div>
                    <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Resume Controls</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      <button
                        type="button"
                        onClick={() => handleHomeBaseCommand(homeBaseSummary.resumeTarget)}
                        style={createButton({ compact: true, tone: homeBaseSummary.resumeTarget?.tone || "success" })}
                      >
                        {homeBaseSummary.resumeTarget?.actionLabel || "Resume Best"}
                      </button>
                      {rankedResumeOptions.slice(1, 4).map((option) => (
                        <button
                          key={option.key}
                          type="button"
                          onClick={() => handleHomeBaseCommand(option)}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
                  {rankedHomeBaseContexts.map((item) => (
                    <div key={item.key} style={{ ...createSurface({ level: "elevated", accent: item.tone, padding: 12, radius: "lg" }) }}>
                      <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>
                        {item.rank}. {item.label}
                      </div>
                      <div style={{ fontSize: 20, fontWeight: 900, marginTop: 8, color: toneMap[item.tone] || deskTheme.colors.text }}>
                        {item.score}%
                      </div>
                      <div style={{ color: "#f8fafc", marginTop: 6, fontWeight: 800 }}>{item.state}</div>
                      <div style={{ color: deskTheme.colors.soft, marginTop: 6, fontSize: 12, lineHeight: 1.5 }}>{item.detail}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: "warning", padding: 12, radius: "lg" }), marginTop: 14, display: "grid", gap: 10 }}>
                <div>
                  <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Operator Command Rail</div>
                  <div style={{ color: deskTheme.colors.soft, marginTop: 6, lineHeight: 1.6 }}>
                    Ranked desk actions and module switches reuse the shared Home Base command spine across the whole product shell.
                  </div>
                </div>
                  ...
                  {rankedQuickActions.slice(0, 3).map((action) => (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <span style={createPillStyle(action.tone)}>{action.actionLabel}</span>
                      </div>
                      <div style={{ color: "#f8fafc", fontWeight: 800, marginTop: 8 }}>{action.focusLabel}</div>
                      <div style={{ color: deskTheme.colors.soft, marginTop: 6, lineHeight: 1.55 }}>{action.detail}</div>
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, marginTop: 10 }}>
          ...
          <div style={{ ...createSurface({ level: "elevated", accent: continuityInsightPosture.tone, padding: 14, radius: "lg" }) }}>
            ...
            <div style={{ color: deskTheme.colors.soft, marginTop: 8, fontSize: 12, lineHeight: 1.55 }}>
              {latestContinuityInsightCollection?.summary || continuityInsightPosture.summary}
            </div>
            {latestContinuityInsightCollection?.notes || latestContinuityInsightCollection?.checkpoints?.length ? (
              <div style={{ color: deskTheme.colors.soft, marginTop: 8, fontSize: 12, lineHeight: 1.55 }}>
                {latestContinuityInsightCollection?.notes || "No collection notes"} | {(latestContinuityInsightCollection?.checkpoints || []).length} checkpoints
              </div>
            ) : null}
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12, alignItems: "center" }}>
              ...
              {promotedContinuityCheckpoints.length ? (
                <span style={createPillStyle("warning")}>
                  {promotedContinuityCheckpoints.length} promoted checkpoints
                </span>
              ) : null}
              {latestContinuityInsightCollection ? (
                <span style={createPillStyle("success")}>
                  {latestContinuityInsightCollection.name} | {latestContinuityInsightCollection.summary || `${(latestContinuityInsightCollection.reportIds || []).length} reports`}
                </span>
              ) : null}
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, marginTop: 10 }}>
          ...
          <div style={{ ...createSurface({ level: "elevated", accent: pinnedContinuityReports.length || pinnedContinuityPairs.length || pinnedContinuityCollections.length || promotedContinuityCheckpoints.length ? "info" : "slate", padding: 14, radius: "lg" }) }}>
            <div style={{ color: deskTheme.colors.soft, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.6, fontWeight: 800 }}>Pinned Continuity Favorites</div>
            <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
              <div>
                <div style={{ color: deskTheme.colors.soft, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Reports</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  {pinnedContinuityReports.length ? (
                    pinnedContinuityReports.slice(0, 3).map((report) => (
                      <button key={report.id} type="button" onClick={() => handleOpenContinuityInsightReport(report)} style={createButton({ compact: true, tone: "info" })}>
                        {report.name}
                      </button>
                    ))
                  ) : (
                    <span style={createPillStyle("neutral")}>No pinned reports</span>
                  )}
                </div>
              </div>

              <div>
                <div style={{ color: deskTheme.colors.soft, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Pairs</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  {pinnedContinuityPairs.length ? (
                    pinnedContinuityPairs.slice(0, 3).map((pair) => (
                      <button key={pair.id} type="button" onClick={() => handleOpenPinnedContinuityPair(pair)} style={createButton({ compact: true, tone: "warning" })}>
                        {pair.label}
                      </button>
                    ))
                  ) : (
                    <span style={createPillStyle("neutral")}>No pinned pairs</span>
                  )}
                </div>
              </div>

              <div>
                <div style={{ color: deskTheme.colors.soft, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Checkpoints</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  {promotedContinuityCheckpoints.length ? (
                    promotedContinuityCheckpoints.slice(0, 3).map((promotion) => (
                      <button
                        key={promotion.id}
                        type="button"
                        onClick={() => handleOpenPromotedContinuityCheckpoint(promotion)}
                        style={createButton({ compact: true, tone: "warning" })}
                      >
                        {promotion.label}
                      </button>
                    ...
                    <span style={createPillStyle("neutral")}>No promoted checkpoints</span>
                  )}
                </div>
              </div>

              <div>
                <div style={{ color: deskTheme.colors.soft, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Collections</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  {pinnedContinuityCollections.length ? (
                    pinnedContinuityCollections.slice(0, 3).map((collection) => (
                      <button key={collection.id} type="button" onClick={() => handleOpenPinnedContinuityCollection(collection)} style={createButton({ compact: true, tone: "success" })}>
                        {collection.name}
                      </button>
                    ))
                  ) : (
                    <span style={createPillStyle("neutral")}>No pinned collections</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <div style={{ width: "min(840px, 100%)", ...createSurface({ level: "hero", accent: "info", padding: 22, radius: "xxl" }) }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
              <div style={{ ...createSurface({ level: "elevated", accent: "warning", padding: 16, radius: "xl" }) }}>
                <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: deskTheme.colors.soft, fontWeight: 800 }}>Ranked Desk Actions</div>
                <div style={{ color: deskTheme.colors.soft, marginTop: 8, lineHeight: 1.55 }}>
                  The strongest next commands are pinned to the top of the launcher for faster desk control.
                </div>
                <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                  {rankedQuickActions.map((action) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        handleHomeBaseCommand(action);
                        setPaletteOpen(false);
                      }}
                      style={{
                        ...createSurface({ level: "elevated", accent: action.tone, padding: 14, radius: "lg" }),
                        textAlign: "left",
                        color: "inherit",
                        cursor: "pointer"
                      }}
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: "success", padding: 16, radius: "xl" }) }}>
                <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: deskTheme.colors.soft, fontWeight: 800 }}>Resume Best Context</div>
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 8 }}>
                  {homeBaseSummary.resumeTarget?.focusLabel || "Execution Center"}
                </div>
                <div style={{ color: deskTheme.colors.soft, marginTop: 8, lineHeight: 1.55 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
                  ...
                  {rankedResumeOptions.slice(1, 5).map((option) => (
                    <button
                      key={option.key}
                      type="button"
                      onClick={() => {
                        handleHomeBaseCommand(option);
                        setPaletteOpen(false);
                      }}
                      style={createButton({ compact: true, tone: option.tone })}
                    >
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: "info", padding: 16, radius: "xl" }) }}>
                ...
                <div style={{ color: deskTheme.colors.soft, marginTop: 8, lineHeight: 1.55 }}>
                  Switch into the most relevant product lanes first instead of hunting through a flat module list.
                </div>
                <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                  {rankedModuleSwitches.map((module) => (
                    <Link
                      key={module.href}
                      href={module.href}
                      style={{ ...createSurface({ level: "elevated", accent: module.tone, padding: 14, radius: "lg" }), color: "inherit", textDecoration: "none" }}
                      onClick={() => setPaletteOpen(false)}
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        <strong>{module.rank}. {module.label}</strong>
                        <span style={createPillStyle(module.tone)}>{module.priority}%</span>
                      </div>
            ...
            <div style={{ display: "grid", gap: 12, marginTop: 16 }}>
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: "info", padding: 16, radius: "xl" }) }}>
                ...
                {latestContinuityInsightCollection ? (
                  <div style={{ color: deskTheme.colors.soft, marginTop: 8, lineHeight: 1.55 }}>
                    Latest collection: {latestContinuityInsightCollection.name} | {latestContinuityInsightCollection.summary || `${(latestContinuityInsightCollection.reportIds || []).length} reports`}
                  </div>
                ) : null}
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
                  <button
              ...
              <div style={{ ...createSurface({ level: "elevated", accent: "warning", padding: 16, radius: "xl" }) }}>
                <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: deskTheme.colors.soft, fontWeight: 800 }}>Pinned Continuity Favorites</div>
                <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                  <div>
                    <div style={{ color: deskTheme.colors.soft, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.4, fontWeight: 800 }}>Reports</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      {pinnedContinuityReports.slice(0, 3).map((report) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            handleOpenContinuityInsightReport(report);
                            setPaletteOpen(false);
                          }}
                          style={createButton({ compact: true, tone: "info" })}
                        >
                      ...
                      {!pinnedContinuityReports.length ? <span style={createPillStyle("neutral")}>No pinned reports</span> : null}
                    </div>
                  </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      {pinnedContinuityPairs.slice(0, 3).map((pair) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            handleOpenPinnedContinuityPair(pair);
                            setPaletteOpen(false);
                          }}
                          style={createButton({ compact: true, tone: "warning" })}
                        >
                      ...
                      {!pinnedContinuityPairs.length ? <span style={createPillStyle("neutral")}>No pinned pairs</span> : null}
                    </div>
                  </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      {promotedContinuityCheckpoints.slice(0, 3).map((promotion) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            handleOpenPromotedContinuityCheckpoint(promotion);
                            setPaletteOpen(false);
                          }}
                          style={createButton({ compact: true, tone: "warning" })}
                        >
                      ...
                      {!promotedContinuityCheckpoints.length ? <span style={createPillStyle("neutral")}>No promoted checkpoints</span> : null}
                    </div>
                  </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                      {pinnedContinuityCollections.slice(0, 3).map((collection) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            handleOpenPinnedContinuityCollection(collection);
                            setPaletteOpen(false);
                          }}
                          style={createButton({ compact: true, tone: "success" })}
                        >
                      ...
                      {!pinnedContinuityCollections.length ? <span style={createPillStyle("neutral")}>No pinned collections</span> : null}
                    </div>
                  </div>
                </div>
              </div>
