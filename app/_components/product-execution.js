...
import {
  getCryptoCorePaperExecutionFlags,
  resolveCryptoCoreSymbol
} from "../../lib/tpm-crypto-core-contract.mjs";

const cryptoCorePaperExecutionFlags = getCryptoCorePaperExecutionFlags();
const defaultExecutionProviderKey = cryptoCorePaperExecutionFlags.executionProviderKey || "paper";
...
export function getDefaultExecutionProvider() {
  return defaultExecutionProviderKey;
}
...
export function getExecutionPolicyForSymbol(symbol = "") {
  const cryptoSymbol = resolveCryptoCoreSymbol(symbol);

  if (!cryptoSymbol) {
    return {
      symbol,
      assetClass: "Mixed",
      executionProviderKey: defaultExecutionProviderKey,
      providerMode: "paper-only",
  ...
  return {
    ...
    assetClass: cryptoSymbol.assetClass,
    marketKey: cryptoSymbol.marketKey,
    groupKeys: cryptoSymbol.groupKeys || [],
    executionProviderKey: cryptoCorePaperExecutionFlags.executionProviderKey,
    executionMode: cryptoCorePaperExecutionFlags.mode,
    providerMode: cryptoSymbol.providerMode,
    paperSafeExecution: cryptoSymbol.paperSafeExecution,
    liveExchangeConnectivityEnabled: cryptoSymbol.liveExchangeConnectivityEnabled,
    realMoneyExecutionEnabled: cryptoSymbol.realMoneyExecutionEnabled
  };
}
...
export function buildExecutionStatus({
  ...
  const tone =
    provider.key !== defaultExecutionProviderKey
      ? "warning"
      : rejectedCount > 0
        ? "danger"
...
export function runExecutionAction({
  ...
  if (providerKey !== defaultExecutionProviderKey) {
    return {
      recentActions: helpers.pushAction(
        current.recentActions,
