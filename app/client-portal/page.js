...
export default function Page(){
  const data = readJson(path.join(process.cwd(), ".tpm", "client-runtime.json"), {
    overallProgress: 95,
    completed: 95,
    remaining: 5,
    domains: { client:95, notifications:92, operator:94 },
    feeds: []
  });
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Client Portal</h1>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:16}}>
          ...
          <div style={card}><div style={{color:"#94a3b8",fontSize:12}}>Operator Bridge</div><div style={{fontSize:28,fontWeight:900,marginTop:6}}>{data.domains.operator}%</div></div>
        </div>

        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Client Feeds</div>
