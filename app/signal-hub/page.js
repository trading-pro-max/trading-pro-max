...
export default function Page(){
  const data = readJson(path.join(process.cwd(), "data", "signals", "hub.json"), {
    channels:[]
  });
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Signal Hub</h1>
          <div style={{marginTop:10,color:"#94a3b8"}}>Central signal intelligence wall</div>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:16}}>
          {(data.channels || []).map((x)=>(
            <div key={x.slug} style={card}>
              <div style={{fontSize:12,color:"#94a3b8"}}>{x.title}</div>
              <div style={{fontSize:28,fontWeight:900,marginTop:8}}>{x.confidence}%</div>
              <div style={{marginTop:6,color:"#22c55e",fontWeight:900}}>{x.status}</div>
            </div>
          ))}
        </div>
      </div>
