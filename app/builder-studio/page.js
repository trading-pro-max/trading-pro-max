    ...
    <main style={{ minHeight:"100vh", background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)", color:"white", padding:24, fontFamily:"Arial, sans-serif" }}>
      <div style={{ maxWidth:1600, margin:"0 auto", display:"grid", gap:20 }}>
        <div style={c}>
          ...
          <h1 style={{ fontSize:42, margin:"10px 0 0" }}>Builder Studio</h1>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr .95fr", gap:16 }}>
          <div style={c}>
            <div style={{ fontSize:22, fontWeight:900, marginBottom:12 }}>Latest Builder Cycles</div>
            <div style={{ display:"grid", gap:10 }}>
              {(data.logs || []).slice(0, 12).map((x, i) => (
                <div key={i} style={b}>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                    <div style={{ fontWeight:900 }}>Cycle {x.cycle || "-"}</div>
                    <div style={{ color:"#94a3b8", fontSize:12 }}>{x.time ? new Date(x.time).toLocaleString() : "-"}</div>
                  </div>
                  <div style={{ marginTop:6, color:"#cbd5e1" }}>
                    Active: <b>{x.activeModule?.title || "IDLE"}</b> · Progress: <b>{x.avgProgress || 0}%</b> · Readiness: <b>{x.avgReadiness || 0}%</b>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={c}>
            <div style={{ fontSize:22, fontWeight:900, marginBottom:12 }}>Run Control</div>
            <div style={{ display:"grid", gap:10 }}>
              <div style={b}>Start: <b>powershell -ExecutionPolicy Bypass -File .\scripts\tpm-builder-start.ps1</b></div>
              <div style={b}>Stop: <b>powershell -ExecutionPolicy Bypass -File .\scripts\tpm-builder-stop.ps1</b></div>
              <div style={b}>Status: <b>powershell -ExecutionPolicy Bypass -File .\scripts\tpm-builder-status.ps1</b></div>
              <div style={b}>API: <b>/api/builder/status</b></div>
            </div>
          </div>
        </div>

        <div style={c}>
          <div style={{ fontSize:22, fontWeight:900, marginBottom:12 }}>Module Grid</div>
