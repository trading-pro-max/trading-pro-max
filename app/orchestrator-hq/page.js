...
export default function Page(){
  const runtime=readJson(path.join(process.cwd(),".tpm","orchestrator-runtime.json"),{overallProgress:0,domains:{}});
  const plan=readJson(path.join(process.cwd(),"data","orchestrator","plan.json"),{missions:[],scores:{}});
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Orchestrator HQ</h1>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:16}}>
          <div style={card}>
            <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Plan Missions</div>
            <div style={{display:"grid",gap:10}}>
              {(plan.missions || []).map((x)=>(
                <div key={x.slug} style={box}>
                  ...
                  <div style={{marginTop:6,color:"#94a3b8",fontSize:12}}>{x.priority} · {x.owner}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={card}>
            <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Scores</div>
