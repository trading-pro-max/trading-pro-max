...
export default function Page(){
  const data=readJson(path.join(process.cwd(),"data","orchestrator","knowledge.json"),{nodes:[],maps:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Knowledge Grid</h1>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:16}}>
          {(data.nodes || []).map((x)=>(
            <div key={x.slug} style={box}>
              ...
              <div style={{marginTop:8,color:"#22c55e",fontWeight:900,fontSize:24}}>{x.density}%</div>
