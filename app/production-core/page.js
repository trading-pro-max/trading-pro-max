...
export default function ProductionCorePage() {
  const data = getAutonomousCoreStatus();
  const c = card();
  const b = box();
    ...
    <main style={{ minHeight:"100vh", background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)", color:"white", padding:24, fontFamily:"Arial, sans-serif" }}>
      <div style={{ maxWidth:1600, margin:"0 auto", display:"grid", gap:20 }}>
        <div style={c}>
          ...
          <h1 style={{ fontSize:42, margin:"10px 0 0" }}>Autonomous Production Core</h1>
          <div style={{ marginTop:10, color:"#94a3b8" }}>
            Global Progress: {data.globalProgress}% · Remaining: {data.remaining}% · Jump: {data.jumpEarned}/{data.jumpTotal}
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(6,minmax(0,1fr))", gap:16 }}>
          ...
          <div style={c}><div style={{ color:"#94a3b8", fontSize:12 }}>Jump</div><div style={{ fontSize:28, fontWeight:900, marginTop:6 }}>{data.jumpEarned}/{data.jumpTotal}</div></div>
          ...
          <div style={c}><div style={{ color:"#94a3b8", fontSize:12 }}>Checks Closed</div><div style={{ fontSize:28, fontWeight:900, marginTop:6 }}>{data.checks.filter(x => x.pass).length}/{data.checks.length}</div></div>
        </div>

        <div style={c}>
          <div style={{ fontSize:22, fontWeight:900, marginBottom:12 }}>Promotion Checks</div>
