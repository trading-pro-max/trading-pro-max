...
export default function Page(){
  const runtime=readJson(path.join(process.cwd(),".tpm","titan-runtime.json"),{overallProgress:0,domains:{}});
  const data=readJson(path.join(process.cwd(),"data","titan","infinity-kernel.json"),{cards:[],metrics:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Infinity Kernel</h1>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,minmax(0,1fr))",gap:12}}>
          {Object.entries(runtime.domains || {}).map(([k,v])=>(
            <div key={k} style={box}>
              ...
              <div style={{fontSize:24,fontWeight:900,marginTop:6}}>{v}%</div>
            </div>
          ))}
        </div>

        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Kernel Cards</div>
