...
export default function Page(){
  const data = readJson(path.join(process.cwd(), ".tpm", "strategy-runtime.json"), {
    overallProgress: 68,
    completed: 68,
    remaining: 32,
    domains: { core:72, surfaces:70, autonomy:88, deployment:35, expansion:25 },
    executionQueue: [],
    recommendations: []
  });
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Strategy Lab</h1>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:16}}>
          <div style={card}><div style={{color:"#94a3b8",fontSize:12}}>Core</div><div style={{fontSize:28,fontWeight:900,marginTop:6}}>{data.domains.core}%</div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.15fr 1fr",gap:16}}>
            ...
            <div style={{display:"grid",gap:10}}>
              {(data.executionQueue || []).map((x)=>(
                <div key={x.slug} style={box}>
                  ...
                  <div style={{marginTop:6,color:"#cbd5e1"}}>{x.impact}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={card}>
            <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Recommendations</div>
            <div style={{display:"grid",gap:10}}>
              {(data.recommendations || []).map((x,i)=><div key={i} style={box}>{x}</div>)}
            </div>
          </div>
        </div>
      </div>
