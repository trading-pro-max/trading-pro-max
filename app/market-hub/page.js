...
export default function Page(){
  const data=readJson(path.join(process.cwd(),"data","market","stream.json"),{feeds:[]});
  const runtime=readJson(path.join(process.cwd(),".tpm","market-runtime.json"),{overallProgress:0,domains:{stream:0,execution:0,risk:0,orchestration:0,telemetry:0}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Market Hub</h1>
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Feeds</div>
