...
export default function Page(){
  const data=readJson(path.join(process.cwd(),".tpm","command-runtime.json"),{overallProgress:0,domains:{}});
  const mesh=readJson(path.join(process.cwd(),"data","health","mesh.json"),{nodes:[]});
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Command Mesh</h1>
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Mesh Health</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12}}>
            {(mesh.nodes || []).map((x)=>(
              <div key={x.slug} style={box}>
                <div style={{fontWeight:900}}>{x.title}</div>
                <div style={{marginTop:8,color:"#22c55e",fontWeight:900,fontSize:24}}>{x.health}%</div>
