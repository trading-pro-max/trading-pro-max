...
export default function Page(){
  const runtime=readJson(path.join(process.cwd(),".tpm","broker-runtime.json"),{overallProgress:0,domains:{}});
  const broker=readJson(path.join(process.cwd(),"data","broker","runtime.json"),{channels:[],host:"",port:""});
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Broker Bridge</h1>
          <div style={{marginTop:10,color:"#94a3b8"}}>Host: {broker.host} · Port: {broker.port}</div>
        </div>
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Channels</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12}}>
            {(broker.channels || []).map((x)=>(
