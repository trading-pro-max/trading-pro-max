...
export default function Page(){
  const data=readJson(path.join(process.cwd(),"data","supremacy","zero-touch.json"),{operations:[],metrics:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Zero-Touch Ops</h1>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:16}}>
          {(data.operations || []).map((x)=>(
            <div key={x.slug} style={box}>
              ...
              <div style={{marginTop:8,color:"#22c55e",fontWeight:900,fontSize:24}}>{Math.round(x.score)}%</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12}}>
            {Object.entries(data.metrics || {}).map(([k,v])=>(
              <div key={k} style={box}>
                ...
                <div style={{fontSize:28,fontWeight:900,marginTop:6}}>{Math.round(v)}%</div>
              </div>
            ))}
          </div>
        </div>
