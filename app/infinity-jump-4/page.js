...
function R(f,d){try{if(fs.existsSync(f))return JSON.parse(fs.readFileSync(f,"utf8"))}catch{}return d}
...
export default function Page(){
  const rt=R(path.join(process.cwd(),".tpm","jump4-runtime.json"),{overallProgress:0,domains:{}});
  const data=R(path.join(process.cwd(),"data","infinity","jump4.json"),{cards:[],metrics:{}});
  return (
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Infinity Jump 4</h1>
          <div style={{marginTop:10,color:"#94a3b8"}}>Overall: {rt.overallProgress}%</div>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:16}}>
          {Object.entries(rt.domains||{}).map(([k,v])=>(
        ...
        <div style={card}>
          <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Cards</div>
