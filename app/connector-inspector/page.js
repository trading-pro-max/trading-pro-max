...
export default function Page(){
  const runtime=readJson(path.join(process.cwd(),".tpm","runtimeops-runtime.json"),{overallProgress:0,domains:{}});
  const data=readJson(path.join(process.cwd(),"data","runtimeops","connectors.json"),{providers:[],metrics:{}});
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Connector Inspector</h1>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:12}}>
            {(data.providers || []).map((x)=>(
              <div key={x.slug} style={box}>
                ...
                <div style={{marginTop:8,color:x.ready ? "#22c55e" : "#f59e0b",fontWeight:900,fontSize:24}}>{x.ready ? "READY" : "MISSING"}</div>
