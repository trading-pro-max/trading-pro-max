...
export default function Page(){
  const runtime=readJson(path.join(process.cwd(),".tpm","agentmesh-runtime.json"),{overallProgress:0,domains:{}});
  const mesh=readJson(path.join(process.cwd(),"data","agents","mesh.json"),{nodes:[],bridges:{}});
  const tasks=readJson(path.join(process.cwd(),"data","agents","tasks.json"),{queue:[]});
    ...
    <main style={{minHeight:"100vh",background:"linear-gradient(180deg,#020617 0%,#0b1120 100%)",color:"white",padding:24,fontFamily:"Arial,sans-serif"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"grid",gap:20}}>
        <div style={card}>
          ...
          <h1 style={{fontSize:42,margin:"10px 0 0"}}>Agent Mesh</h1>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:12}}>
            {(mesh.nodes || []).map((x)=>(
              <div key={x.slug} style={box}>
                ...
                <div style={{marginTop:8,color:"#22c55e",fontWeight:900,fontSize:24}}>{x.load}%</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div style={card}>
            <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Bridges</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:12}}>
              {Object.entries(mesh.bridges || {}).map(([k,v])=>(
                <div key={k} style={box}>
                  <div style={{color:"#94a3b8",fontSize:12}}>{k}</div>
                  <div style={{fontSize:28,fontWeight:900,marginTop:6}}>{v}%</div>
                </div>
          ...
          <div style={card}>
            <div style={{fontSize:22,fontWeight:900,marginBottom:12}}>Task Queue</div>
            <div style={{display:"grid",gap:10}}>
              {(tasks.queue || []).map((x)=>(
                <div key={x.slug} style={box}>
                  ...
                  <div style={{marginTop:6,color:"#94a3b8",fontSize:12}}>{x.owner} · {x.status}</div>
                </div>
              ))}
            </div>
          </div>
